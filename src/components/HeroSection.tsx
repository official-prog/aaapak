  ...
  return (
    <>
      <section className="relative w-full overflow-hidden">
        {/* Height wrapper: fixed min-height on mobile so content never clips; auto on md+ */}
        <div className="relative min-h-[520px] sm:min-h-[580px] md:min-h-0 md:h-auto">

          {/* Banner image — object-cover on mobile, natural h-auto on desktop */}
          <img
            src={heroBanner}
          ...
          {/* Gradient overlay — stronger on mobile for full text readability */}
          <div
            className="absolute inset-0"
            style={{
              background:
                "linear-gradient(to right, rgba(255,255,255,0.98) 0%, rgba(255,255,255,0.95) 40%, rgba(255,255,255,0.75) 62%, rgba(255,255,255,0.2) 82%, transparent 100%)",
            }}
          />

          {/* Content overlay */}
          <div className="absolute inset-0 flex items-center pt-20 pb-8 md:pt-16 md:pb-8">
            <div className="container">
              <div className="w-[72%] sm:w-auto sm:max-w-[420px] md:max-w-[480px]">
                ...
                <h1 className="font-black text-foreground">
                  <span className="block text-lg sm:text-2xl md:text-3xl lg:text-[2.4rem] leading-tight">
                    Custom Packaging Sourcing
                  </span>
                  <span className="block text-lg sm:text-2xl md:text-3xl lg:text-[2.4rem] leading-tight text-primary mt-1 md:mt-2">
                    with Reliable Production Support
                  </span>
                </h1>

                <p className="mt-2 text-xs sm:text-sm md:text-base font-semibold text-foreground/80">
                ...
                <div className="mt-3 md:mt-4 flex flex-col gap-2">
                  <div>
                    <Button
                      size="sm"
                      className="font-semibold px-4 md:px-5 md:h-10 text-xs sm:text-sm"
                      onClick={() => setQuoteOpen(true)}
                    >
                      Get a Free Quote <ArrowRight className="ml-1.5 w-3.5 h-3.5" />
                    </Button>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <Button
                      size="sm"
                      className="font-semibold px-4 md:px-5 bg-[#25D366] hover:bg-[#20b858] text-white border-0 text-xs sm:text-sm"
                      asChild
                    >
                      <a href="https://wa.me/16477788860" target="_blank" rel="noopener noreferrer">
                        <svg viewBox="0 0 32 32" className="w-3.5 h-3.5 fill-white mr-1.5 shrink-0" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="font-semibold px-4 md:px-5 bg-white/80 text-xs sm:text-sm"
                ...
                <div className="mt-3 md:mt-4 space-y-0.5">
                  <p className="text-xs text-muted-foreground hidden sm:block">
                    Trusted by brands across 25+ industries&nbsp;|&nbsp;Global supply chain &amp; reliable delivery
                  </p>
